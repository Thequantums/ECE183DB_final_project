  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (number_of_objects > 0)
    {
      ...
      for (int i = 0; i < number_of_objects; ++i) 
        {
          if (objects[i].colors[0] == 0)
          {
            if (hicounter > 500)
            {
              char message [100]; 
              sprintf(message, "Hippo: Relative position is %.2f %.2f %.2f \n Relative orientation is %.2f %.2f %.2f %.3f ", 
          ...
          else if (objects[i].colors[0] == 1)
          {
            if (hocounter > 500)
            {
            char message [100]; 
            sprintf(message, "Hound: Relative position is %.2f %.2f %.2f \n Relative orientation is %.2f %.2f %.2f %.3f ", 
            objects[i].position[0], objects[i].position[1], objects[i].position[2],
            objects[i].orientation[0], objects[i].orientation[1], objects[i].orientation[2], objects[i].orientation[3]);
            wb_emitter_send(emitter, message, strlen(message) + 1);
            printf("I sent: \"%s\" \n\n", message);
    ...
    if (counter > 700)
    {
      ...
      {
        const char *message = "Hound: I hate you";
        wb_emitter_send(emitter, message, strlen(message) + 1);
        printf("I sent: \"%s\" \n", message);
        counter = 0;
        who = 1;
